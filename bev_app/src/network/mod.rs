...
use prost::Message;
use protocol::{frame::{create_function_frame, parse_function_frame}, generated::{client::ConnectionState, msg::{ClientHandlers, Presentation}}};
...
pub fn send_message<T: prost::Message>(socket: &mut ResMut<MatchboxSocket>, peer_ids: Vec<String>, msg_type: ClientHandlers, payload: &T) -> Result<(), ()> {
...
pub fn receive_messages(
    ...
    for (peer, state) in socket.update_peers() {
        match state {
            PeerState::Connected => {
                peers_info.peers.insert(peer.0.to_string(), PeerInfo { position: None, other_player: None, state: ConnectionState::Connected });

                let presentation = my_player_to_presentation(&my_player_resource);

                send_message(&mut socket, vec![peer.0.to_string()], ClientHandlers::Presentation, &presentation)
                    .unwrap();
            },
        ...
        if let Ok((id, data)) = parse_function_frame(&message) {
            ...
            match id {
               ClientHandlers::Presentation => {
                ...
                ev_player_connection.send(PlayerConnectionEvent { peer_info: new_peer_info, is_reconnect: reconnect });
