...
impl SelectedRoom {
   ...
   pub fn create_my_room(my_player: &MyPlayerResource) -> Self {
    ...
    Self { name: format!("{}-{}", my_player.player.name, unique_name) }
   }
}
...
pub fn generate_rool_url(
    ...
    format!("{}/{}?name={}&hash={}&pub_key={}", server.url, room.name, my_player.player.name, my_player.player.hash, URL_SAFE.encode(my_player.player.pub_key.as_bytes()))
}