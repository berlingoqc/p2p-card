...
pub fn decrypt_with_key(ciphertext: &[u8], key: &Key<Aes256Gcm>) -> Result<Vec<u8>, ()> {
    ...
    Ok(cipher.decrypt(nonce, &ciphertext[12..]).unwrap())
}
...
mod tests {
    ...
    use crate::logic::{
        deck::encryption::{generate_random_nonce, get_encrypted_card_nonce},
        players::{key_loader::get_key_loader, MyPlayerConfiguration},
    };
