...
use crate::logic::{
    deck::encryption::{
        encrypt_cards_from_deck, encrypt_shares_for_player, get_encrypted_card_nonce,
        to_encrypted_card, to_own_shared, EncryptedCard, EncryptedCards, SharedPrivateCardShares,
    },
    players::{MyPlayer, OtherPlayer},
};
...
impl PrivatePlayerGameState {
    ...
    pub fn new(my_player: &MyPlayer, other_players: &Vec<OtherPlayer>) -> Self {
        ...
        for player in other_players.iter() {
            sealing_keys.insert(
                player.hash,
                SealingKey::create(&my_player.private, player.pub_key.clone()),
            );
        }
    ...
    pub fn generate_starting_data(
        &mut self,
        starting_deck: &Vec<Vec<u8>>,
        indexes: &Vec<u32>,
        threshold: u8,
    ) -> Result<(HashMap<u64, SharedPrivateCardShares>, EncryptedCards), ()> {
        let mut private_encrypted_cards = encrypt_cards_from_deck(
            starting_deck,
            indexes,
            (self.sealing_keys.len() + 1) as u8,
            threshold,
        )?;

        let mut players_shares: HashMap<u64, SharedPrivateCardShares> =
            HashMap::with_capacity(self.sealing_keys.len());

        for player_id in self.sealing_keys.keys() {
            let sealing_key = self.sealing_keys.get(player_id).unwrap();
            players_shares.insert(
                *player_id,
                encrypt_shares_for_player(&mut private_encrypted_cards, sealing_key)?,
            );
        }
        ...
        Ok((
            players_shares,
            private_encrypted_cards
                .iter()
                .map(to_encrypted_card)
                .collect(),
        ))
    }

    // Add the data receive by the other player , your share for the card the generated
    pub fn add_other_player_starting_data(
        &mut self,
        other_player: u64,
        shared: &SharedPrivateCardShares,
    ) -> Result<(), ()> {
        ...
        let items = share_request
            ...
            .map(|nonce| {
                ...
                (
                    nonce.clone(),
                    sealing_key
                        .encrypt(nonce, &share.as_bytes().to_vec())
                        .unwrap(),
                )
            })
            .collect();
    ...
    pub fn add_other_player_shares(
        &mut self,
        other_player: u64,
