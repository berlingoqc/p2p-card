...
mod tests {
    ...
    use protocol::generated::card::{
        AgreementDefinition, DeckEncyption, PlayerEncryption, PlayingArea, PlayingField, Vec3,
    };

    use crate::logic::{
        deck::encryption::get_encrypted_card_nonce,
        encryption::private::{PrivatePlayerGameState, ShareRequest},
        players::{MyPlayer, MyPlayerConfiguration},
    };
    ...
    fn load_alice() {
        ...
        let john_config = MyPlayerConfiguration {
        ...
        let game_configuration = GameConfiguration {
            ...
            deck_encryption: Some(DeckEncyption {
                ...
                reconstruction_order: vec![alice.hash, john.hash],
                player_encryption: vec![
                    PlayerEncryption {
                        player_name: alice.hash,
                        indexes: alice_indexes.clone(),
                    },
                    PlayerEncryption {
                        player_name: john.hash,
                        indexes: john_indexes.clone(),
                    },
                ],
            }),
            playing_field: None,
            players: vec![alice.to_protoc_player(), john.to_protoc_player()],
            ..Default::default()
        };
        ...
        let alice_starting_data = alice_game_state
            .generate_starting_data(
                &starting_deck,
                &alice_indexes,
                game_configuration
        ...
        let john_starting_data = john_game_state
            .generate_starting_data(
                &starting_deck,
                &john_indexes,
                game_configuration
        ...
        alice_game_state
            .add_other_player_starting_data(
                john.hash,
                john_starting_data.0.get(&alice.hash).unwrap(),
            )
            .unwrap();
        john_game_state
            .add_other_player_starting_data(
                alice.hash,
                alice_starting_data.0.get(&john.hash).unwrap(),
            )
            .unwrap();

        //assert_eq!(agreed_shared_game_state..starting_deck.cards.len(), shared_game_state.deck.cards.len());
